...
from ._exceptions import CaesarError, APIStatusError
...
class Caesar(SyncAPIClient):
    ...
    def __init__(
        ...
        *,
        ...
        """Construct a new synchronous Caesar client instance.

        This automatically infers the `api_key` argument from the `CAESAR_API_KEY` environment variable if it is not provided.
        """
        if api_key is None:
            api_key = os.environ.get("CAESAR_API_KEY")
        if api_key is None:
            raise CaesarError(
                "The api_key client option must be set either by passing api_key to the client or by setting the CAESAR_API_KEY environment variable"
            )
        self.api_key = api_key

        if base_url is None:
            base_url = os.environ.get("CAESAR_BASE_URL")
        if base_url is None:
            base_url = f"https://api.caesar.xyz"
...
class AsyncCaesar(AsyncAPIClient):
    ...
    def __init__(
        ...
        *,
        ...
        """Construct a new async AsyncCaesar client instance.
